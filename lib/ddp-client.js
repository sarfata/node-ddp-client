...
DDPClient.prototype._prepareHandlers = function() {
  ...
  self.socket.on('close', function(code, message) {
    if (self._callbacks.closed)
      self._callbacks.closed(code, message);
  });
 ...
 *  error(error): Called if an error occurs during connection or on the socket
 *  closed(code, message): Called when the connection is closed
 */
DDPClient.prototype.connect = function(connected, error, closed) {
  var self = this;
...
// Register a filter to incoming messages
DDPClient.prototype.onMessage = function (filter) {
  this._msg_filters.push(filter);
};
